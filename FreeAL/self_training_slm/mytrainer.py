...
from torch.nn import BCEWithLogitsLoss, CrossEntropyLoss, KLDivLoss, NLLLoss
...
from utils.utils import metrics_fn, path_checker, set_logger
...
class MyTrainer(Trainer):
    def _inner_training_loop(
        ...
        if not args.ignore_data_skip:
            for epoch in range(epochs_trained):
                ...
                if not is_random_sampler:  # or is_torch_less_than_1_11
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            if epoch > 0:
                ...
                loss_all = (
                    (
                        (loss_all - loss_all[valid_idx_all].min())
                        / (loss_all[valid_idx_all].max() - loss_all[valid_idx_all].min())
                    )
                    .detach()
                    .cpu()
                    .reshape(-1, 1)
                ...
                for j in range(num_labels):
                    ...
                    for k in range(num_clusters):
                        ...
                        embedding_all_representative.append(
                            vectors_in_cluster[index_of_representative]
                        )
                        idx_all_representative.append(
                            idx_in_cluster[index_of_representative].reshape(1)
                        )
                    ...
                    cos_similarities = cosine_similarity(
                        embeddings_all, embedding_all_representative
                    )
                    sort_result = torch.sort(
                        torch.from_numpy(cos_similarities), dim=1, descending=True
                    )
                ...
                if args.learning_setting == "transductive" and epoch == 1:
                    ...
                    # clean sample index
                    with open("./feedback/right_list_massive.pkl", "wb") as f:
                        pickle.dump(chosen_idx_sel.tolist(), f)
                    # demonstration retrieval
                    with open("./feedback/demo_index_massive.pkl", "wb") as f:
                        pickle.dump(chosen_top_indices.tolist(), f)
                    # pseudo-labels for all samples
                    with open("./feedback/pred_label_massive.pkl", "wb") as f:
            ...
            # self._maybe_log_save_evaluate(tr_loss, model, trial, epoch, ignore_keys_for_eval) # TODO check if we needed extra logs
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        embeddings = model.roberta(**myinputs)[0]  # .roberta
        ...
        # embedding-level mix-up strategy on clean subset
        valid_embeddings = embeddings[index_x]
        with torch.no_grad():
            targets_l = F.one_hot(
                labels[index_x].detach(),
                num_classes=self.num_labels if hasattr(self, "num_labels") else 10,
            )  # TODO: find the proper fix, hard-coded num_classes!
            all_targets = targets_l.detach()
        rand_idx = torch.randperm(valid_embeddings.size(0))
        l_term = np.random.beta(4, 4)
        l_term = max(l_term, 1 - l_term)
        mixed_embeddings = l_term * valid_embeddings + (1 - l_term) * valid_embeddings[rand_idx]
        mixed_targets = l_term * all_targets + (1 - l_term) * all_targets[rand_idx]
        mixed_logits = model.classifier(mixed_embeddings)
        loss_mix = -torch.mean(
            torch.sum(F.log_softmax(mixed_logits, dim=-1) * mixed_targets, dim=-1)
        )
        ...
        if is_in_train:
            ...
            embeddings_bt = model.roberta(**myinputs_bt)[0]  # .roberta TODO: check with BERT
            logits_bt = model.classifier(embeddings_bt)
            loss_cr = loss_fct(logits_bt[index_x], labels.view(-1)[index_x]).mean()
            loss_cr_un = 0.5 * loss_fct_un(
                F.log_softmax(logits[index_u] / self.temp_u, dim=-1),
                F.softmax(logits_bt[index_u].detach().clone(), dim=-1),
            ) + 0.5 * loss_fct_un(
