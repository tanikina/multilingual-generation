        ...
        with FileLock(lock_path):
            if os.path.exists(cached_features_file) and not overwrite_cache:
                ...
                self.features = convert_examples_to_features(
                    examples, max_seq_length, tokenizer, mode
                )
    ...
    def _create_examples(self, corpus, mode):
        ...
        for data in tqdm.tqdm(corpus, desc="creating examples for " + mode):
            ...
            if mode == "train_chat":
                contents_bt = data[
                    2
                ]  # pipe_bt(pipe(data[0])[0]["translation_text"])[0]["translation_text"]
            else:
                contents_bt = None
            examples.append(
                DatasetInputExample(
            ...
            features.append(
                {
                    ...
                    "inputs_bt": DatasetInputFeature(
                        index=ex_index,
                        input_ids=inputs_bt.input_ids,
                        attention_mask=inputs_bt.attention_mask
                        if "attention_mask" in inputs_bt
                        else None,
                        token_type_ids=None,
                        label=example.label,
                    ),
